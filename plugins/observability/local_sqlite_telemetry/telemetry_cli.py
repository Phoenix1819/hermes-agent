#!/usr/bin/env python3
"""Telemetry CLI — query ~/.hermes/profiles/phoenix/telemetry.db
...
_DEFAULT_DB = os.path.expanduser("~/.hermes/profiles/phoenix/telemetry.db")
...
def report(days: int = 7):
    ...
    with _db() as conn:
        ...
        cur = conn.execute("""
            ...
            WHERE timestamp >= ?
        """, (since,))
        avg_ctx, max_ctx, compress = cur.fetchone()
    lines = [
